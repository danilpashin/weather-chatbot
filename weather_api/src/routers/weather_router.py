...
@router.get(
    "/health",
    dependencies=[
        Depends(RateLimiter(limiter=Limiter(Rate(1000, Duration.MINUTE * 1))))
    ],
)
...
async def get_weather(
    ...
    try:
        ...
        logger.exception(
            (
                f"❌ Ошибка при получении погоды | город={city} | query={raw_name}"
                f"| ip={request.client.host} | error={type(e).__name__}"
            )
        )
