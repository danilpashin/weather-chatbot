...
class PostgresDB(Database):
    ...
    async def get_user_data(self, user_id: int) -> Any | None:
        async with self.engine.begin() as conn:
            result = await conn.execute(text("SELECT city FROM users WHERE id = :id"), {"id": user_id})
    ...
    async def set_user_data(self, user_id: int, city: str) -> None:
        async with self.engine.begin() as conn:
            ...
            if data is None:
                await conn.execute(text("INSERT INTO users (id, city) VALUES (:id, :city)"), {"id": user_id, "city": city})
            elif data[0] != city:
                await conn.execute(text("UPDATE users SET city = :city WHERE id = :id"), {"id": user_id, "city": city})
            return None
